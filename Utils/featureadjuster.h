...
// An detector adjuster optimized for image sequences (video).
...
public:
    ...
    virtual void detect(cv::InputArray _image, std::vector<cv::KeyPoint>& keypoints, cv::InputArray _mask = cv::noArray());
...
public:
    ...
    VideoGridAdaptedFeatureDetector(const cv::Ptr<StatefulFeatureDetector>& detector,
        int maxTotalKeypoints = 1000, int gridRows = 4, int gridCols = 4, int edgeThreshold = 31);
